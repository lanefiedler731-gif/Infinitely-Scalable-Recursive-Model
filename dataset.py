...
class DatasetConfig:
    ...
    datasets: List[str] = field(default_factory=lambda: [
        ...
        "TeichAI/minimax-m2.1-1000x",
        "TeichAI/claude-4.5-opus-high-reasoning-250x",
        "open-thoughts/OpenThoughts3-1.2M",  # Large reasoning dataset (1.2M examples)
        "a-m-team/AM-DeepSeek-R1-0528-Distilled", # DeepSeek R1 Distilled
    ])
    ...
    seed: int = 42
    max_length: int = 2048
    max_samples_per_dataset: Optional[int] = None  # Limit samples per dataset (None = all)
    num_proc: int = 32 # Default processes for map/filtering
    max_download_speed_mb: Optional[int] = None # Max download speed in MB/s (None = unlimited)
...
class DatasetLoader:
    ...
    def _load_large_dataset_streaming(self, dataset_name: str) -> Optional[Dataset]:
        ...
        # Generator that yields examples from the stream
        # This allows writing directly to disk without intermediate huge file
        def stream_generator():
            ...
            for example in stream_ds:
                ...
                if max_bytes_per_sec:
                    ...
                    # We use a simple heuristic for text/json size
                    example_bytes = 0
                ...
                # Sanitize DeepSeek info to prevent null/string type mismatches
                if is_deepseek:
                    clean_example = {}
                    if 'conversations' in example:
                        clean_convs = []
        ...
        try:
            ...
            # Use features from stream if available to avoid inference overhead
            features = stream_ds.features 
            
            # Create dataset from generator - this writes to cache_dir
            # We specify cache_dir to ensure it writes to our location
            # Note: We don't pass features=stream_ds.features because sometimes the HF metadata
            # specifies NullType for columns that actually contain data (causing cast errors)
    ...
    def _load_standard_dataset(self, dataset_name: str) -> Optional[Dataset]:
        ...
        # Get the train split (or the only split if there's just one)
    ...
    def normalize_format(self, dataset: Dataset, source_name: str) -> Dataset:
        ...
        def process_example(example: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if 'conversations' in example:
                ...
                for turn in convs:
                    if isinstance(turn, dict):
                        ...
                        if role in ['human', 'user', 'Human']:
                            ...
                            # User requested to not include system messages
                            continue
                        
                        # Check for thinking/answer in info (DeepSeek format)
                        if 'info' in turn and isinstance(turn['info'], dict):
                            info = turn['info']
                ...
                # Open-Orca format (ignoring system_prompt as requested)
                # content is in 'question', output in 'response'
                conversations = [
    ...
    def run(self) -> DatasetDict:
        ...
        for i, (ds, name) in enumerate(zip(raw_datasets, self.config.datasets)):
            source_name = name.split('/')[-1]  # Just the dataset name
...
class ConversationTokenizer:
    ...
    def format_conversation(self, conversations: List[Dict[str, str]]) -> Tuple[str, List[Tuple[int, int]]]:
        ...
        for turn in conversations:
            ...
            if role == 'assistant':
                # Track where assistant content starts (after the role line)
    ...
    def tokenize_with_mask(
        ...
        # Create label mask based on character positions
        ...
        for i, (start, end) in enumerate(offsets):
            ...
            # Check if this token is within any assistant span
            for span_start, span_end in assistant_spans:
                if start >= span_start and end <= span_end:
                    label_mask[i] = 1.0
                    break
        
        # Labels are same as input_ids (next-token prediction uses shifted indices)
...
class TrainingDataset(TorchDataset):
    ...
    def __init__(
        ...
        # Define processing function for map
        def process_batch(batch):
            ...
            for conversations in batch['conversations']:
                ...
                try:
                    ...
                    # Convert to numpy for Arrow storage
        ...
        # Use provided num_proc
        safe_num_proc = num_proc
        if safe_num_proc > 1:
            print(f"Using {safe_num_proc} processes for tokenization")
        
        # Apply tokenization using map (memory efficient)
        self.dataset = hf_dataset.map(
            process_batch,
            batched=True,
            batch_size=1000,  # Larger batches for multiprocessing efficiency
            num_proc=safe_num_proc,  # Use requested number of processes
...
def create_dataloaders(
    ...
    num_proc: int = 32,  # Default to 32 threads for tokenization
    ...
    # Create PyTorch datasets with caching
    ...
    # ========================================================================
    # These optimizations speed up data loading without affecting training.
    # None of these change the data itself or the model training process.
    
    # Determine optimal settings based on num_workers
    use_persistent_workers = num_workers > 0
    # prefetch_factor: number of batches to pre-load per worker
    # Higher values reduce GPU idle time waiting for data
    prefetch_factor = 4 if num_workers > 0 else None
    
    # Create DataLoaders with static shapes (important for CUDA graphs)
    # + Performance optimizations that don't affect training quality:
    train_loader = DataLoader(
        ...
        drop_last=True,   # Important for CUDA graphs - need consistent batch size
        persistent_workers=use_persistent_workers,  # Keep workers alive between epochs (avoids restart overhead)
        prefetch_factor=prefetch_factor,  # Pre-queue batches to minimize GPU idle time
    )
